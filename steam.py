...
class Steam():
  def __init__(self):
    ...
    lastlog = parsed.get("response", {}).get("players", [{}])[0].get("lastlogoff", None)
    ...
    if onstatus == 1:
      ...
      bar = "Looking to play/Trade"         # SteamAPI gives variable online status, this sorts the value of key into a response
    else:
      bar = "Offline/Private"


    print(f"Steam64 ID: {steamID} \n PersonaName: {pname} \n Link to full avatar:{avatar}\n Real-Name: {fname}\n Profile URL: {pfurl}\n Online Status: {bar}\n Account creation: {created}\n Last Steam Log off: {logoff}")
    foo = input("\nType 'yes' to load csgo stats\n")
    if 'yes' in foo:
